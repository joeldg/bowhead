...
class BitfinexWebsocketCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->instrument = 'BTCUSD';
        $loop = \React\EventLoop\Factory::create();
        $connector = new \Ratchet\Client\Connector($loop);

        $connector('wss://api.bitfinex.com/ws')
            ->then(function(\Ratchet\Client\WebSocket $conn) {
                $conn->send('{"event": "subscribe","channel":"trades","pair": "'. $this->instrument .'"}');
                $conn->send('{"event": "subscribe","channel":"ticker","pair": "'. $this->instrument .'"}');
                $conn->send('{"event": "subscribe","channel":"book","pair": "'. $this->instrument .'","prec":"R0","freq":"F0"}');
