...
trait Strategies
{
    protected $strategies_all = [
         'bowhead_sar_stoch', 'bowhead_awesome_macd', 'bowhead_adx_smas', 'bowhead_rsi_macd', 'bowhead_sar_rsi', 'bowhead_stoch_adx', 'bowhead_cci_scalper', 'bowhead_ema_scalper', 'bowhead_ema_stoch_rsi', 'bowhead_double_volatility', 'bowhead_adx_momentum', 'bowhead_base_150', 'bowhead_breakout_ma', 'bowhead_sar_awesome', 'bowhead_cci_ema', 'bowhead_bband_rsi', 'bowhead_ema_adx_macd', 'bowhead_mov_avg_sar', 'bowhead_momentum', 'bowhead_sma_stoch_rsi'

        // not organized into time series yet...
        // need to test them more..
        , 'bowhead_sar_sma', 'bowhead_ema_adx', 'bowhead_trend_bounce', 'bowhead_5th_element', 'bowhead_powerranger', 'bowhead_famamama',
    ];

    protected $strategies_1m = [
         'bowhead_sar_stoch', 'bowhead_awesome_macd', 'bowhead_adx_smas', 'bowhead_rsi_macd', 'bowhead_sar_rsi', 'bowhead_ema_scalper',
    ];

    protected $strategies_5m = [
         'bowhead_stoch_adx', 'bowhead_cci_scalper', 'bowhead_adx_momentum',
    ];

    protected $strategies_15m = [
         'bowhead_double_volatility', 'bowhead_bband_rsi',
    ];

    protected $strategies_30m = ['bowhead_sar_awesome'];

    protected $strategies_1h = [
         'bowhead_ema_stoch_rsi', 'bowhead_base_150', 'bowhead_breakout_ma' // technically this is a 1d, but can be used more
        , 'bowhead_cci_ema', 'bowhead_ema_adx_macd' // technically a 4h
        , 'bowhead_mov_avg_sar', 'bowhead_momentum', 'bowhead_sma_stoch_rsi',
    ];
    ...
    public function bowhead_cci_scalper($pair, $data, $return_full = false)
    {
        ...
        if (count($data['close']) < 200) {
            return ['err' => 'need larger data set. (200 min)'];
        }
    ...
    public function bowhead_base_150($pair, $data, $return_full = false)
    {
        if (count($data['close']) < 365) {
            ...
            return ['err' => 'need larger data set. (365 min)'];
        }
